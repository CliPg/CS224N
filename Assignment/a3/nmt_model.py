...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        X = self.post_embed_cnn(X.permute(1, 2, 0))  # (b, e, src_len) -> (b, e, src_len) -> (src_len, b, e)
        ...
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens_packed)  # shape: (src_len, b, 2h)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # shape: (b, src_len, 2h)
        last_hidden_cat = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # shape: (b, 2h)
        init_decoder_hidden = self.h_projection(last_hidden_cat)  # shape: (b, h)
        last_cell_cat = torch.cat((last_cell[0], last_cell[1]), dim=1)  # shape: (b, 2h)
        init_decoder_cell = self.c_projection(last_cell_cat)  # shape: (b, h)
